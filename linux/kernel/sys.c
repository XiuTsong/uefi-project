...
SYSCALL_DEFINE2(setregid, gid_t, rgid, gid_t, egid) {
  ...
  if (egid != (gid_t)-1) {
    if (gid_eq(old->gid, kegid) || gid_eq(old->egid, kegid) ||
        gid_eq(old->sgid, kegid) || ns_capable(old->user_ns, CAP_SETGID))
      new->egid = kegid;
    else
      goto error;
  }
...
SYSCALL_DEFINE2(setreuid, uid_t, ruid, uid_t, euid) {
  ...
  if (euid != (uid_t)-1) {
    ...
    if (!uid_eq(old->uid, keuid) && !uid_eq(old->euid, keuid) &&
        !uid_eq(old->suid, keuid) && !ns_capable(old->user_ns, CAP_SETUID))
      goto error;
  }
...
SYSCALL_DEFINE0(getpgrp) { return sys_getpgid(0); }
